...
function App() {
  ...
  const URL = `${process.env.REACT_APP_API_ENDPOINT}/url`;
    ...
    <div className="App">
      ...
      <main>
        <form onSubmit={createURL}>
          ...
          <button type="submit" id="submit_button" value="Submit">
            {" "}
            Create{" "}
          </button>
          ...
          <h3 id="url">
            {" "}
            <a href={BASE_URL + responseAPI.slug}>
              {" "}
              {BASE_URL + responseAPI.slug}{" "}
            </a>{" "}
          </h3>
          <h3 id="failure">
            {" "}
            Creation failed: check that url is in format http://example.com or
            https://example.com and that the slug is unique!{" "}
          </h3>
        </form>
      </main>
    </div>
